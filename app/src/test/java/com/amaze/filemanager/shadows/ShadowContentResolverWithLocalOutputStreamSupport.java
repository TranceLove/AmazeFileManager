...
public class ShadowContentResolverWithLocalOutputStreamSupport {
  ...
  private void addUriPermission(@NonNull Uri uri, int modeFlags) {
    ReflectionHelpers.ClassParameter<Uri> p1 =
        new ReflectionHelpers.ClassParameter<>(Uri.class, uri);
    ReflectionHelpers.ClassParameter<Integer> p2 =
        new ReflectionHelpers.ClassParameter<>(int.class, modeFlags);
    ReflectionHelpers.ClassParameter<Long> p3 =
        new ReflectionHelpers.ClassParameter<>(long.class, System.currentTimeMillis());
   ...
   *
   * <p>Instead, use ```java ProviderInfo info = new ProviderInfo(); info.authority = authority;
  ...
  /**
   * Set the value to be returned by {@link ContentResolver#update(Uri, ContentValues, String,
   * String[])} when no appropriate {@link ContentProvider} can be found.
   *
